# -*- coding: utf-8 -*-
"""
Created on Sat Dec 14 15:40:30 2024
...
plt.ylabel("")
plt.subplot(1,2,2)
plt.pie(x=df["Kalite"].value_counts(),labels=df["Kalite"].value_counts().index, shadow=True,
        explode=[0.02,0.02],startangle = 30, autopct='%0.2f%%', colors=cols)
plt.legend(labels=["good","bad"], loc=(1,0.8), fontsize="small")
plt.suptitle("hedef değişken sayısı ve oranları", fontsize=16)
plt.tight_layout()
plt.show()
...
for i in model_list:
    ...
    model_name_liste.append(model_name)
